...
const sideProjects = 'https://isrsep.dreamhosters.com/wp-json/wp/v2/side_projects'
...
export async function getSideprojects(){
    const sideProjectRes = await fetch(sideProjects, {
        agent: httpsAgent
    })
    const sideProject = await sideProjectRes.json()
    return sideProject
}
